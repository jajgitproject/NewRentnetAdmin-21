...
export class FormDialogComponent {
  ...
  constructor(
    ...
    if (this.action === 'edit') {
      this.dialogTitle = 'Billing Type Name';
      this.advanceTable = data.advanceTable;
    } else {
      this.dialogTitle = 'Billing Type Name';
      this.advanceTable = new BillingType({});
    ...
    return this.fb.group(
      {
        ...
        billingTypeName: [this.advanceTable.billingTypeName, [this.noWhitespaceValidator]],
      ...
      .subscribe(
        response => {
          if (response && response.activationStatus && typeof response.activationStatus === 'string' && response.activationStatus.includes("Duplicate")) {
            ...
            this._generalService.sendUpdate('BillingTypeUpdate:BillingTypeView:Success');//To Send Updates  
            this.saveDisabled = true;
