...
@Component({
  standalone: false,
  selector: 'app-changeKamForCustomers',
  templateUrl: './changeKamForCustomers.component.html',
  styleUrls: ['./changeKamForCustomers.component.sass'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class ChangeKamForCustomersComponent implements OnInit {
  displayedColumns = [
    'oldKeyAccountManagerName',
    'newKeyAccountManagerName',
    'startDate',
    'updationDateTime',
  ...
  dataSource: ChangeKamForCustomers[] | null;
  changeKamForCustomersID: number;
  advanceTable: ChangeKamForCustomers | null;
  SearchChangeKamForCustomers: string = ''; 
  ...
  SearchEntityType:string='';
   SearchnewKAMActivationStatus : boolean=true;
    SearchnewKAMActivationFromDate:  string = ''; 
  SearchnewKAMActivationToDate: string = ''; 
  PageNumber: number = 0;
  sortType: string;
  sortingData: number;
  ActiveStatus: any;
  ...
  constructor(
    ...
    public changeKamForCustomersService: ChangeKamForCustomersService,
  ...
  refresh() {
    ...
    this.SearchChangeKamForCustomers = '';
    this.SearchEmployeeName='';
    this.SearchStartDate='';
    this.SearchnewKAMActivationStatus = true;
    this.searchTerm = '';
  ...
  editCall(row) {
      //  alert(row.id);
    this.changeKamForCustomersID = row.id;
    const dialogRef = this.dialog.open(FormDialogComponentHolder, {
      data: {
        advanceTable: row,
        action: 'edit',
  ...
  openInNewTab(menuItem: any, rowItem: any) {
    ...
    if(menuItem.label.toLowerCase() === 'customer kam city') {
      const url = this.router.serializeUrl(this.router.createUrlTree(['/customerKAMCity'], { queryParams: {
        changeKamForCustomersID: rowItem.changeKamForCustomersID,
  ...
  public loadData() 
   {
    ...
    switch (this.selectedFilter)
    {
      ...
      this.changeKamForCustomersService.getTableData(this.SearchEmployeeName,this.SearchFromDate,this.SearchActivationStatus, this.PageNumber).subscribe
    (
      data =>   
      {
  ...
  onContextMenu(event: MouseEvent, item: ChangeKamForCustomers) {
...
/////////////////for Image Upload ends////////////////////////////
    ...
    (
      ...
      { 
        ...
        if(this.MessageArray.length==3)
        {
          ...
          else if(this.MessageArray[0]=="ChangeKamForCustomersUpdate")
          {
            if(this.MessageArray[1]=="ChangeKamForCustomersView")
            {
              if(this.MessageArray[2]=="Success")
          ...
          else if(this.MessageArray[0]=="ChangeKamForCustomersDelete")
          {
            if(this.MessageArray[1]=="ChangeKamForCustomersView")
            {
              if(this.MessageArray[2]=="Success")
          ...
          else if(this.MessageArray[0]=="ChangeKamForCustomersAll")
          {
            if(this.MessageArray[1]=="ChangeKamForCustomersView")
            {
  ...
  SortingData(coloumName:any) {
    ...
    this.changeKamForCustomersService.getTableDataSort(this.SearchEmployeeName,this.SearchnewKAMActivationFromDate,this.SearchnewKAMActivationStatus,this.PageNumber, coloumName.active,this.sortType).subscribe
    (
      data =>   
      {
        this.dataSource = data;
