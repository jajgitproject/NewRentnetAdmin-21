...
import { ChangeCarTypeModel, ChangeEntityModel } from './changeCarType.model';
...
@Component({
  standalone: false,
  selector: 'app-changeCarType',
  templateUrl: './changeCarType.component.html',
  styleUrls: ['./changeCarType.component.scss'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class ChangeCarTypeComponent implements OnInit {
  displayedColumns = [
    'actions',
    'ReservationID',
    'DutySlipID',
  ...
  searchDutySlipID:string = '';
    ...
    onContextMenu(event: MouseEvent, item: ChangeCarTypeModel) 
    {
    ...
    onCustomerGroupSelected(customerGroup: string) 
    {
      ...
      this.InitCustomer(selectedCustomerGroup?.customerGroupID);
    }
    ...
    UpdateCarType(row:any) 
    {
    if(row.invoiceCalculationCancelationStatus === false && row.invoiceID !== 0)
    {
        Swal.fire({
        title: 'Car Type cannot be changed as Invoice is already generated for this reservation',
        icon: 'warning',
      ...
      {
     const dialogRef = this.dialog.open(ChangeCarTypeFormDialogComponent, {
      width: '600px',
      data: 
        {         
    ...
    OpenCarTypeDetails(row:any) 
    {
      const dialogRef = this.dialog.open(ChangeCarTypeDetailsComponent, {
      width: '600px',
      data: 
        {         
          reservationID : row.reservationID,
