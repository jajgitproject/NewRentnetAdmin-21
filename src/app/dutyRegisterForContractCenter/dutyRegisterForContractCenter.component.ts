...
export class DutyRegisterForContractCenterComponent implements OnInit {
 baseDisplayedColumns = [
  ...
  downloadFilteredCsv() {
    ...
    const searchCriteria = this.buildSearchCriteria();
    this.dutyRegisterForContractCenterService.exportCsv(searchCriteria).subscribe(
      (blob: Blob) => {
  ...
  private triggerCsvDownload(blob: Blob) {
    ...
    link.href = url;
    link.download = `DutyRegisterForContractCenter_${timeStamp}.csv`;
