...
import { DutyRegisterForContractCenterService } from './dutyRegisterForContractCenter.service';
import { HttpClient, HttpErrorResponse } from '@angular/common/http';
import { MatDialog } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { DutyRegisterModel, SalesPersonDropDownModel, SearchCriteria } from './dutyRegisterForContractCenter.model';
...
@Component({
  standalone: false,
  selector: 'app-duty-register-for-contract-center',
  templateUrl: './dutyRegisterForContractCenter.component.html',
  styleUrls: ['./dutyRegisterForContractCenter.component.sass'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class DutyRegisterForContractCenterComponent implements OnInit, OnDestroy {
  ...
  private readonly exportJobPageKey = 'dutyRegisterForContractCenter';
 ...
 baseDisplayedColumns = [
  ...
  columnFieldMap: Record<string, string> = {
    ...
    KAM: 'kam',
    ReservationCreatedBy: 'reservationCreatedBy'
  };
  ...
  constructor(
    ...
    public dutyRegisterForContractCenterService: DutyRegisterForContractCenterService,
  ...
  //   }
  //   this.dutyRegisterForContractCenterService.getTableData(this.SearchCustomerGroup.value,this.SearchCustomerPerson.value,this.SearchDutyType.value,this.SearchFeedbackDate,
  ...
  public loadData() 
  {
    ...
    this.dutyRegisterForContractCenterService.getTableData(searchCriteria,this.PageNumber,).subscribe(
      data =>   
      {
        const rows = this.applyCustomerSpecificFieldColumns(Array.isArray(data) ? data : []);
  ...
  startExportJob() {
    ...
    this.dutyRegisterForContractCenterService.startExportJob(searchCriteria).subscribe(
      (startResult: any) => {
  ...
  downloadExportCsv() {
    if (!this.exportJobId || !this.dutyRegisterForContractCenterService.isExportJobReady(this.exportJobStatus) || this.exportJobDownloading) {
      return;
    }

    this.exportJobDownloading = true;
    this.dutyRegisterForContractCenterService.downloadExportJob(this.exportJobId).subscribe(
      async (blob: Blob) => {
  ...
  canDownloadExport(): boolean {
    return !!this.exportJobId && this.dutyRegisterForContractCenterService.isExportJobReady(this.exportJobStatus) && !this.exportJobDownloading;
  }

  isExportJobInProgress(): boolean {
    return this.exportJobRunning || this.dutyRegisterForContractCenterService.isExportJobRunning(this.exportJobStatus);
  }
  ...
  private startExportPolling(jobId: string) {
    this.stopExportPolling();
    this.exportPollSub = this.dutyRegisterForContractCenterService.pollExportJob(jobId).subscribe(
      (status: any) => {
        this.exportJobStatus = status;
  ...
  private resumeExportJobIfNeeded() {
    ...
    this.dutyRegisterForContractCenterService.getExportJobStatus(jobId).subscribe(
      (status: any) => {
        if (!status) {
          this.exportJobRunning = true;
          this.startExportPolling(jobId);
        ...
        this.exportJobStatus = status;
        this.exportJobError = '';
        if (this.dutyRegisterForContractCenterService.isExportJobRunning(status)) {
          this.exportJobRunning = true;
          this.exportJobStartedAt = markExportDumpStarted(this.exportJobStartedAt, this.exportJobStatus);
          this.startExportPolling(jobId);
          return;
  ...
  private triggerCsvDownload(blob: Blob, preferredFileName?: string) {
    ...
    link.href = url;
    link.download = preferredFileName || `DutyRegisterForContractCenter_${timeStamp}.csv`;
  ...
  {   
    ...
    const searchCriteria = this.buildSearchCriteria();
    this.dutyRegisterForContractCenterService.getTableDataSort(searchCriteria,this.PageNumber,coloumName.active,this.sortType).subscribe
    (
      data =>   
      {
        const rows = this.applyCustomerSpecificFieldColumns(Array.isArray(data) ? data : []);
  ...
  InitCustomerGroup()
  {
    this.dutyRegisterForContractCenterService.GetCustomerGroupByKAMForDutyRegister(this.IsKAMRole).subscribe(
    data=>
    {
  ...
  InitCustomer()
  {
    this.dutyRegisterForContractCenterService.GetCustomerByKAMForDutyRegister(this.IsKAMRole).subscribe(
    data=>
    {
  ...
  {
    this.dutyRegisterForContractCenterService.GetLocationDropDownForDutyRegister(this.IsKAMRole).subscribe(
    data=>
    {
 ...
 //---------- Location Group ---------
  ...
  getCellDisplayValue(row: DutyRegisterModel, columnId: string): string {
    const field = this.columnFieldMap[columnId] || columnId;
  ...
  private updateColumnWidths(rows: DutyRegisterModel[]) {
    const padding = 32;
    const minWidth = 72;
    const maxWidth = 420;
    const charPx = 7.2;
