...
@Component({
  standalone: false,
  selector: 'app-change-collection-executive-dialog',
  templateUrl: './change-collection-executive-dialog.component.html',
  styleUrls: ['./change-collection-executive-dialog.component.sass']
})
export class ChangeCollectionExecutiveDialogComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ChangeCollectionExecutiveDialogComponent>,
    ...
    this.changeForm = this.fb.group({
      newCollectionExecutive: ['', Validators.required]
    });
  }
  ...
  ngOnInit(): void {
    this._generalService.GetEmployeesForVehicleCategory().subscribe({
      next: (data) => {
        ...
        const control = this.changeForm.controls['newCollectionExecutive'];
  ...
  onChange() {
    ...
    Swal.fire({
      title: 'Are you sure?',
      html: `<b>Change Collection Executive for ${this.selectedCount} selected record(s)?</b><br/>Existing assignments will be deactivated and new records will be created.`,
      icon: 'warning',
