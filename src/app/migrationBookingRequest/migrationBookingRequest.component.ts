...
import { MigrationBookingRequestService } from './migrationBookingRequest.service';
import { MigrationBookingRequest } from './migrationBookingRequest.model';
...
@Component({
  standalone: false,
  selector: 'app-migrationBookingRequest',
  templateUrl: './migrationBookingRequest.component.html',
  styleUrls: ['./migrationBookingRequest.component.sass'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class MigrationBookingRequestComponent implements OnInit {
  columnDefinitions = [
  ...
  actionRow: MigrationBookingRequest | null = null;

  dataSource: MigrationBookingRequest[] | null;
  ...
  constructor(
    ...
    public migrationBookingRequestService: MigrationBookingRequestService,
  ...
  setActionRow(row: MigrationBookingRequest): void {
    this.actionRow = row;
  }
  ...
  downloadTable(): void {
    ...
    const rows = this.dataSource.map(row => [
      ...
      row.reservationID ? `${row.reservationGroupID || ''}${row.reservationGroupID ? '.' : ''}${row.reservationID}` : '',
  ...
  public loadData()
  {
    ...
    this.migrationBookingRequestService.getTableData(fromDate,toDate,this.SearchTRN,this.SearchiTRN,this.customerGroupName.value,this.SearchEcoBookingNo,this.getConfirmByEcoApiValue(),this.PageNumber).subscribe(
      data => {
  ...
  private handleLoadError(error: HttpErrorResponse): void {
    ...
    let message = error?.error?.message
      || error?.message
      || (error?.status ? `Could not load migration booking requests (${error.status}).` : 'Could not load migration booking requests.');
    if (error?.status === 404) {
      message = 'migrationBookingRequest API not found. Stop IIS Express, rebuild RententAPI, and start debugging again.';
    } else if (error?.status === 0) {
      message = 'Cannot reach the API at ' + (this._generalService.BaseURL || '') + '. Start RententAPI (IIS Express on port 44368).';
    }
  ...
  onContextMenu(event: MouseEvent, item: MigrationBookingRequest) {
  ...
  SortingData(coloumName: any) 
  {
    ...
    const { fromDate, toDate } = this.getFormattedSearchDates();
    this.migrationBookingRequestService.getTableDataSort(fromDate, toDate, this.SearchTRN, this.SearchiTRN, this.customerGroupName.value, this.SearchEcoBookingNo, this.getConfirmByEcoApiValue(), this.PageNumber, coloumName.active, this.sortType).subscribe(
        data => {
          this.dataSource = data;
        },
        (error: HttpErrorResponse) => this.handleLoadError(error)
      );
  }
  ...
  openInNewTab(rowItem: any) {
    ...
    const url = this.router.serializeUrl(this.router.createUrlTree(['/migrationBookingConfiguration'], { queryParams: {
      BookingID: rowItem.integrationRequestID,
      returnUrl: '/migrationBookingRequest',
      } }));
