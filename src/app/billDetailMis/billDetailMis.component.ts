...
@Component({
  standalone: false,
  selector: 'app-bill-detail-mis',
  templateUrl: './billDetailMis.component.html',
  styleUrls: ['./billDetailMis.component.sass'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class BillDetailMisComponent implements OnInit, OnDestroy {
  ...
  private readonly exportJobPageKey = 'billDetailMis';
  ...
  constructor(
    ...
    public billDetailMisService: BillDetailMisService
  ) {
  ...
  SearchData() {
    ...
    this.billDetailMisService.startExportJob(searchCriteria).subscribe(
      (startResult: any) => {
  ...
  downloadExportCsv() {
    if (!this.exportJobId || !this.billDetailMisService.isExportJobReady(this.exportJobStatus) || this.exportJobDownloading) {
      return;
    }

    this.exportJobDownloading = true;
    this.billDetailMisService.downloadExportJob(this.exportJobId).subscribe(
      async (blob: Blob) => {
  ...
  cancelExportJob() {
    ...
    this.billDetailMisService.cancelExportJob(this.exportJobId).subscribe(
      (status: any) => {
  ...
  canDownloadExport(): boolean {
    return !!this.exportJobId && this.billDetailMisService.isExportJobReady(this.exportJobStatus) && !this.exportJobDownloading;
  }

  isExportJobInProgress(): boolean {
    return this.exportJobRunning || this.billDetailMisService.isExportJobRunning(this.exportJobStatus);
  }
  ...
  private startExportPolling(jobId: string) {
    this.stopExportPolling();
    this.exportPollSub = this.billDetailMisService.pollExportJob(jobId).subscribe(
      (status: any) => {
        this.exportJobStatus = status;
  ...
  private triggerCsvDownload(blob: Blob, preferredFileName?: string) {
    ...
    link.href = url;
    link.download = preferredFileName || `billDetailMis_${timeStamp}.csv`;
  ...
  private initCustomerGroup() {
    this.billDetailMisService.GetCustomerGroupByKAMForDutyRegister(this.IsKAMRole).subscribe((data) => {
  ...
  private initCustomer() {
    this.billDetailMisService.GetCustomerByKAMForDutyRegister(this.IsKAMRole).subscribe((data) => {
  ...
  private initDispatchLocation() {
    this.billDetailMisService.GetLocationDropDownForDutyRegister(this.IsKAMRole).subscribe((data) => {
