...
export interface DynamicsMisTableColumn {
  id: string;
  key: string;
}
...
@Component({
  standalone: false,
  selector: 'app-dynamicsMis',
  templateUrl: './dynamicsMis.component.html',
  styleUrls: ['./dynamicsMis.component.sass'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class DynamicsMisComponent implements OnInit {
  tableColumns: DynamicsMisTableColumn[] = [];
  ...
  constructor(
    public dynamicsMisService: DynamicsMisService,
  ...
  downloadCsv() {
    ...
    this.dynamicsMisService.downloadCsv(fromDate, toDate, customer, invoiceNo, branch).subscribe(
      (blob: Blob) => {
        ...
        a.href = fileUrl;
        a.download = `DynamicsMIS_${moment().format('YYYYMMDD_HHmmss')}.csv`;
