...
export class ChangeEntityModel {
  ...
  userID: number;
  checked:boolean;
  invoiceCalculationCancelationStatus: boolean;
  invoiceID: number;
...
export class ChangeCarTypeModel {
  ...
  reason:string;
  userID: number;
  constructor(changeCarTypeModel) {
    {
      this.reservationChangeLogID = changeCarTypeModel.reservationChangeLogID || -1;
      this.reservationID = changeCarTypeModel.reservationID || '';
      this.changeEmployeeID = changeCarTypeModel.changeEmployeeID || '';
      this.changeType = changeCarTypeModel.changeType || '';      
      this.previousRecordID = changeCarTypeModel.previousRecordID || '';
      this.previousRecordName = changeCarTypeModel.previousRecordName || ''; 
      this.newRecordID = changeCarTypeModel.newRecordID || '';
      this.newRecordName = changeCarTypeModel.newRecordName || '';
      this.reason = changeCarTypeModel.reason || '';
    }
  }
}
