...
import { EmployeeDemo } from '../../employeeDemo.model';
import { MAT_DATE_LOCALE } from '@angular/material/core';
import { CommonModule, formatDate } from '@angular/common';
import { GeneralService } from '../../../general/general.service';
import { EmployeeDemoDropDown, LocationDropDown } from '../../employeeDemoDropDown.model';
...
export class FormDialogComponent
{
  ...
  public EmployeesList?: EmployeeDemoDropDown[] = [];
  filteredEmployeesOptions: Observable<EmployeeDemoDropDown[]>;
  ...
  filteredEmployeeOptions: Observable<EmployeeDemoDropDown[]>;
  ...
  filteredLocationOptions: Observable<LocationDropDown []>;
   public EmployeesList2: EmployeeDemoDropDown[] = [];
filteredEmployeeOptions2!: Observable<EmployeeDemoDropDown[]>;
 public RoleList2?: RoleDropDown[] = [];
 filteredRoleOptions2!: Observable<RoleDropDown[]>;
  ...
  public advanceTableService: EmployeeDemoService,
    ...
    dateOfLeaving: this.advanceTable.dateOfLeaving
      ...
      : null
 ...
 else 
        {
          ...
          this.dialogTitle = 'Employee';
          this.advanceTable = new EmployeeDemo({});
...
//   }
 ...
 supplierTypeValidator(SupplierList: any[]): ValidatorFn {
  ...
  OnRoleSelect2(selectedRole: string)
  {
    const RoleName = this.RoleList.find(
      data => data.role === selectedRole
    );
...
InitEmployee2() {
  this._generalService.GetReportingManager().subscribe(data => {
    this.EmployeesList2 = data;

    this.advanceTableForm.controls['reportingManager'].setValidators([
      Validators.required,
      this.employeeValidator(this.EmployeesList2)
    ]);

    this.advanceTableForm.controls['reportingManager'].updateValueAndValidity();

    this.filteredEmployeeOptions2 =
      this.advanceTableForm.controls['reportingManager'].valueChanges.pipe(
        startWith(''),
        map(value => this._filterEmployee2(value || ''))
      );

    if (this.action === 'edit' && this.advanceTable) {
      const rm2 = this.EmployeesList2.find(
        e => e.employeeID === this.advanceTable.reportingManagerID
      );

      if (rm2) {
        const fullValue =
          `${rm2.firstName} ${rm2.lastName} ${rm2.employeeOfficeID}`;

        this.advanceTableForm.patchValue({
          reportingManager2: fullValue
        });
      }
    }
  });
...
private _filterEmployee2(value: string): any[] {
  ...
  return this.EmployeesList2.filter(option =>
    (`${option.firstName} ${option.lastName} ${option.employeeOfficeID}`)
      .toLowerCase()
      .includes(filterValue)
  );
}

OnReportingManager2Select(value: string) {
  const employee = this.EmployeesList2.find(
    x => `${x.firstName} ${x.lastName} ${x.employeeOfficeID}` === value
  );

  if (employee) {
    this.advanceTableForm.patchValue({
      reportingManager2ID: employee.employeeID
    });
  }
}
