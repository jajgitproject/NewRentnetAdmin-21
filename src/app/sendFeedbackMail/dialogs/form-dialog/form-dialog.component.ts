...
import { SendFeedbackMail,ConfigurationMessaging } from '../../sendFeedbackMail.model';
...
export class FormDialogSendFeedbackMailComponent {
  ...
  previousData: SendFeedbackMail[];
  dispatchCurrentData: SendFeedbackMail[];
  dispatchNextData: SendFeedbackMail[];
  ...
  // apiRequestData: any;
  constructor(
    public dialogRef: MatDialogRef<FormDialogSendFeedbackMailComponent>,

    @Inject(MAT_DIALOG_DATA) public data: any,
    public advanceTableService: SendFeedbackMailService,
    public sendFeedbackMailService:SendFeedbackMailService,
    ...
    this.advanceTable = new SendFeedbackMail({});
    ...
    this.advanceTableService.add(this.advanceTableForm.getRawValue()).subscribe(
      (response) => {
        ...
        this._generalService.sendUpdate(
          'SendFeedbackMailCreate:SendFeedbackMailView:Success'
        ); //To Send Updates
      },
      (error) => {
        this._generalService.sendUpdate(
          'SendFeedbackMailAll:SendFeedbackMailView:Failure'
        ); //To Send Updates
      }
    );
  }
    ...
    this.advanceTableService
      ...
      .subscribe(
        (response) => {
          this.dialogRef.close();
          this._generalService.sendUpdate(
            'SendFeedbackMailUpdate:SendFeedbackMailView:Success'
          ); //To Send Updates
          this.showNotification(
            'snackbar-success',
            'SendFeedbackMail Updated...!!!',
            'bottom',
            'center'
          );
        },
  ...
  sendNotification() {
    ...
    this.permissionData?.forEach((element) => {
      ...
      apiRequestData.push({
        ...
        IsCustomerNotificationsAllowed:
          element.reservationSMSToBooker === true
            ? true
            : false || element.resevationSMSToPassenger === true
            ? true
            : false,
    ...
    this.notificationloadData( apiRequestData);
  }

  notificationloadData( additionalData: any) {
    this.sendFeedbackMailService
