...
@Component({
  standalone: false,
  selector: 'app-cdcOutStationRoundTripRate',
  templateUrl: './cdcOutStationRoundTripRate.component.html',
  styleUrls: ['./cdcOutStationRoundTripRate.component.sass'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class CDCOutStationRoundTripRateComponent implements OnInit {
  displayedColumns = [
  ...
  dataSource: CDCOutStationRoundTripRate[] | null;
  cdcOutStationRoundTripRateID: number;
  CDCOutStationRoundTripRateID: number=0;
  advanceTable: CDCOutStationRoundTripRate | null;
  ...
  constructor(
    ...
    public cdcOutStationRoundTripRateService: CDCOutStationRoundTripRateService,
...
InitVehicleCategory(){
  ...
  refresh() {
    ...
    this.CDCOutStationRoundTripRateID=0;
  ...
  editCall(row) {
      //  alert(row.id);
    this.cdcOutStationRoundTripRateID = row.cdcLocalRateID;
    const dialogRef = this.dialog.open(FormDialogComponent, {
      data: {
        advanceTable: row,
        action: 'edit',
  ...
  deleteItem(row)
  {
    ...
    this.cdcOutStationRoundTripRateID = row.id;
  ...
  onBackPress(event) 
  {
   ...
   public loadData() 
   {
    switch (this.selectedFilter)
    {
      ...
      this.cdcOutStationRoundTripRateService.getTableData(
        this.CDCOutStationRoundTripRateID,
  ...
  onContextMenu(event: MouseEvent, item: CDCOutStationRoundTripRate) {
    event.preventDefault();
...
/////////////////for Image Upload ends////////////////////////////
  ...
  SubscribeUpdateService()
  {
    ...
    (
      ...
      { 
        ...
        {
          if(this.MessageArray[0]=="CDCOutStationRoundTripRateCreate")
          {
            if(this.MessageArray[1]=="CDCOutStationRoundTripRateView")
            {
              if(this.MessageArray[2]=="Success")
              {
                ...
                'snackbar-success',
                'CDC Outstation Round Trip Rate Created ...!!!',
                'bottom',
                'center'
              );
              }
          ...
          else if(this.MessageArray[0]=="CDCOutStationRoundTripRateUpdate")
          {
            if(this.MessageArray[1]=="CDCOutStationRoundTripRateView")
            {
              if(this.MessageArray[2]=="Success")
              {
               ...
               this.showNotification(
                'snackbar-success',
                'CDC Outstation Round Trip Rate Updated ...!!!',
                'bottom',
                'center'
              );
              }
          ...
          else if(this.MessageArray[0]=="CDCOutStationRoundTripRateDelete")
          {
            if(this.MessageArray[1]=="CDCOutStationRoundTripRateView")
            {
              if(this.MessageArray[2]=="Success")
              {
               ...
               this.showNotification(
                'snackbar-success',
                'CDC Outstation Round Trip Rate Deleted ...!!!',
                'bottom',
                'center'
              );
              }
          ...
          else if(this.MessageArray[0]=="CDCOutStationRoundTripRateAll")
          {
            if(this.MessageArray[1]=="CDCOutStationRoundTripRateView")
            {
  ...
  SortingData(coloumName:any) {
    ...
    this.cdcOutStationRoundTripRateService.getTableDataSort(
      this.CDCOutStationRoundTripRateID,
      this.customerContract_ID,
  ...
  RateView()
  {
    ...
    const url = this.router.serializeUrl(this.router.createUrlTree(
      ['/rateViewForOutstationRoundTripContract'],
      {
        queryParams: {
