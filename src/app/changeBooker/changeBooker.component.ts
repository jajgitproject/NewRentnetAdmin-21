...
import { ChangeEntityModel } from './changeBooker.model';
import { ChangeBookerDetailsComponent } from './dialogs/changePassengerDetails/changePassengerDetails.component';
import moment from 'moment';

@Component({
  standalone: false,
  selector: 'app-changeBooker',
  templateUrl: './changeBooker.component.html',
  styleUrls: ['./changeBooker.component.scss'],
  providers: [{ provide: MAT_DATE_LOCALE, useValue: 'en-GB' }]
})
export class ChangeBookerComponent implements OnInit {
  displayedColumns = [
    'actions',
    'ReservationID',
    'DutySlipID',
  ...
  searchDutySlipID:string = '';
    ...
    constructor(
      ...
      public changePassengerService: ChangeBookerService,
    ...
    UpdateBooker(row:any) 
    {
      const dialogRef = this.dialog.open(ChangeBookerFormDialogComponent, {
      width: '600px',
      data: 
        {         
          reservationID : row.reservationID,
          customerGroupID : row.customerGroupID,
          primaryBookerID : row.primaryBookerID,
          primaryBooker : row.primaryBooker
        }
      });
    ...
    {
      const dialogRef = this.dialog.open(ChangeBookerDetailsComponent, {
      width: '600px',
      data: 
        {         
          reservationID : row.reservationID,
